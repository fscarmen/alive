  ...
  for h in ${REMOVE_PROXIES[@]}; do
    ...
    if echo "$h" | grep -q "^socks"; then
      ...
      if echo $PROXY_NOW | grep -q 'type=ws'; then
        ...
        WS_PATH=$(echo $PROXY_NOW | sed 's#.*path=\([^#]\+\).*#\1#g; s#%2f#/#g')
        WS_PORT=$(echo $PROXY_NOW | sed "s#.*:\([0-9]\+\)?.*#\1#g")
        
        if echo $PROXY_NOW | grep -q 'security=tls'; then
          # 安全类型为 tls,即为 vless + ws + tls
          JSON="{ \"inbounds\": [ { \"listen\": \"172.20.0.1\", \"port\": $V2RAY_PORT, \"protocol\": \"dokodemo-door\", \"settings\": { \"network\": \"tcp,udp\", \"followRedirect\": true }, \"sniffing\": { \"enabled\": true, \"destOverride\": [ \"http\", \"tls\" ] } } ], \"policy\": { \"levels\": { \"0\": { \"statsUserDownlink\": true, \"statsUserUplink\": true } }, \"system\": { \"statsInboundUplink\": true, \"statsInboundDownlink\": true } }, \"outbounds\": [ { \"mux\": { \"enabled\": false, \"concurrency\": 8 }, \"protocol\": \"vless\", \"streamSettings\": { \"wsSettings\": { \"path\": \"$WS_PATH\", \"headers\": { \"host\": \"$WS_HOST\" } }, \"tlsSettings\": { \"allowInsecure\": false }, \"security\": \"tls\", \"network\": \"ws\" }, \"tag\": \"proxy\", \"settings\": { \"vnext\": [ { \"address\": \"$WS_ADD\", \"users\": [ { \"encryption\": \"none\", \"id\": \"$WS_ID\", \"level\": 0, \"flow\": \"\" } ], \"port\": $WS_PORT } ] } } ], \"routing\": { \"rules\": [ { \"type\": \"field\", \"outboundTag\": \"proxy\", \"source\": [ \"172.20.0.2\" ] }, { \"type\": \"field\", \"network\": \"tcp,udp\", \"outboundTag\": \"direct\" } ] } }"
        else
          # 安全类型为 tls,即为 vless + ws + tcp
          JSON=""
        fi
      fi
    elif
      ...
      JSON="{ \"inbounds\": [ { \"listen\": \"172.20.0.1\", \"port\": $V2RAY_PORT, \"protocol\": \"dokodemo-door\", \"settings\": { \"network\": \"tcp,udp\", \"followRedirect\": true }, \"sniffing\": { \"enabled\": true, \"destOverride\": [ \"http\", \"tls\" ] } } ], \"policy\": { \"levels\": { \"0\": { \"statsUserDownlink\": true, \"statsUserUplink\": true } }, \"system\": { \"statsInboundUplink\": true, \"statsInboundDownlink\": true } }, \"outbounds\": [ { \"protocol\": \"shadowsocks\", \"streamSettings\": { \"network\": \"tcp\", \"tcpSettings\": { \"header\": { \"type\": \"none\" } }, \"security\": \"none\" }, \"tag\": \"proxy\", \"settings\": { \"servers\": [ { \"port\": $SS_PORT, \"method\": \"$SS_METHOD\", \"password\": \"$SS_PASSWORD\", \"address\": \"$SS_ADD\", \"level\": 0, \"email\": \"\", \"ota\": false } ] } }, { \"tag\": \"direct\", \"protocol\": \"freedom\", \"settings\": { \"domainStrategy\": \"UseIP\", \"userLevel\": 0 } } ], \"routing\": { \"rules\": [ { \"type\": \"field\", \"outboundTag\": \"proxy\", \"source\": [ \"172.20.0.2\" ] }, { \"type\": \"field\", \"network\": \"tcp,udp\", \"outboundTag\": \"direct\" } ] } }"
